...
| 3 | **High** | admin cookie HMAC 密钥复用 `admin_password`（口令=签名密钥=旁路凭据三重复用） | `admin.rs:100-118,131-132` | [API 与管理后台 §5](../backend/api-admin.md#5)、[加密链 §6.1](crypto-chain.md) |
| 4 | Medium | bootstrap-owner 常驻后门：知 `admin_password` 即可无用户名登录为 owner，不可禁用 | `admin.rs:336-346` | [API 与管理后台 §5](../backend/api-admin.md#5) |
| 5 | Medium | 明文 HTTP 传输（生产无 TLS）+ 客户端无证书 pinning | `main.rs:79-90`；`http_client.cpp:39` TODO | [加密链 §4.3](crypto-chain.md)、[客户端 · 网络](../client/net-storage-core.md#http) |
| 6 | Medium | 客户端验签 / BLAKE3 校验 / 启动全未实现——信任模型核心承诺无代码兜底 | grep `src/`、`tools/preview-d2d/` 无命中 | [数据流 · 启动游戏](../architecture/data-flow.md#4)、[加密链 §4.2](crypto-chain.md) |
| 7 | Medium | 出货客户端用简化 HWID（ComputerName+UserName+VolumeSerial），非 14 源设计 | `tools/preview-d2d/hwid.h:15-54` | [客户端 · 加密与原生 §5](../client/crypto-native.md#hwid) |
| 8 | Low | HWID/username 仅单遍 SHA-256（盐半公开），泄盐后可离线比对 | `hashing.rs:22-28` | [认证/会话 §2](../backend/auth-session.md) |
| 9 | Low | 登录限流为进程内 `Mutex<HashMap>`，重启即清空、多实例各自计数 | `state.rs:17`, `auth.rs:270-291` | [认证/会话 §4](../backend/auth-session.md#4) |
| 10 | Low | 过期 session 行不自动清理（仅查询谓词过滤） | `auth.rs` 全文无清扫任务 | [认证/会话 §6](../backend/auth-session.md) |
| 11 | Low | 私钥 `signer/private.key` 明文落盘，无口令加密 | `signer/main.rs:81` | [Signer / Proto §1.2](../data/signer-proto.md) |
| 12 | Info | registry.h 注释承诺的 AES-GCM 二次加密未实现，实际 = 单纯 DPAPI(user) | `registry.h:8,29` vs `registry.cpp:99` | [客户端 · 加密与原生 §4](../client/crypto-native.md#4) |

!!! note "已在源码中修复的历史项"
    **rebind 过期 token 旁路**：`rebind::submit` 现在校验 `expires_at > now()`（`rebind.rs:46`），记忆中记录的
    「过期 session token 被 rebind::submit 接受」在当前源码已收敛，与 `rebind.rs:108` 逐字一致。据记忆
    `rebind-expiry-auth-gap`，该修复编译通过后的**部署状态需在 home cloud 再确认**，且提交的 `.sqlx` 离线缓存曾陈旧。

!!! danger "信任模型现状总结"
    设计上「服务器被打穿也伪造不了签名，因为客户端验签」——但**客户端验签当前无代码**（#6）。在落地前，`.helix`
    内容完整性的实际保障仅停留在 signer 一侧；同时 HWID 门禁不强制（#1）、admin 存在 `?key=` 明文引导后门（#2）、
    cookie HMAC 密钥复用口令（#3）。这些是当前而非假想的缺口，随产品路线 Phase D/E 收敛。
