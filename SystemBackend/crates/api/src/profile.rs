...
// =====================================================================
#[derive(Deserialize)]
pub struct ProfileQuery { pub session_token: String }
...
pub async fn change_nickname(
    ...
    let last = sqlx::query_scalar!(
        "SELECT nickname_changed_at FROM users WHERE id=$1", uid)
        .fetch_one(&s.db).await.map_err(internal)?;
...
pub async fn get_avatar(
    ...
    let row = match sqlx::query!(
        "SELECT avatar_path, avatar_mime FROM users WHERE id=$1", id)
    ...
    let mime = row.avatar_mime.unwrap_or_else(|| "application/octet-stream".into());
...
pub async fn login_history(
    ...
    let rows = sqlx::query!(
        r#"SELECT occurred_at, success, failure_reason,
           ...
           FROM login_history
           WHERE user_id=$1
           ORDER BY occurred_at DESC LIMIT 100"#, uid)
        .fetch_all(&s.db).await.map_err(internal)?;

    Ok(Json(rows.into_iter().map(|r| LoginEntry {
